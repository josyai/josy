...
} from '../replanning/variety';
import { checkRecipeMatchesIntent } from '../planning-horizon';
import {
  InvalidInputError,
...
// ─────────────────────────────────────────────────────────────────────────────
// v0.6 Options Interface
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Options for planTonightWithOptions (v0.6 extension).
 */
export interface DPEOptionsV06 {
  /** Recipe slugs to exclude from consideration */
  excludeRecipeSlugs?: string[];
  /** Recent consumption profile for variety penalties */
  varietyProfile?: RecentConsumptionProfile;
  /** Ingredients expiring within urgency threshold */
  expiringIngredients?: Set<string>;
  /** Intent override for this day */
  intentOverride?: IntentOverride;
  /** Shadow inventory for multi-day planning (use instead of DB) */
  shadowInventory?: Array<{
    id: string;
...
// ─────────────────────────────────────────────────────────────────────────────
// v0.6 Extension: planTonightWithOptions
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Plan tonight's dinner with extended options (v0.6).
 *
 * This extends planTonight with:
 * - Recipe exclusions (for no-repeat within horizon)
 * - Variety penalties (from consumption profile)
 * - Intent overrides (must_include, must_exclude, preferred)
 * - Shadow inventory (for multi-day simulation)
 *
 * @param householdId - Household to plan for
 * @param nowTs - Current timestamp (or midpoint for future days)
 * @param calendarBlocksInput - Calendar blocks for this day
 * @param options - v0.6 extended options
 * @returns Complete plan response
 */
export async function planTonightWithOptions(
  householdId: string,
  nowTs: Date,
  calendarBlocksInput: CalendarBlockInput[],
  options: DPEOptionsV06 = {}
): Promise<PlanTonightResponse> {
  const {
    excludeRecipeSlugs = [],
    varietyProfile,
    expiringIngredients = new Set<string>(),
    intentOverride,
    shadowInventory,
  } = options;
  ...
  // ─────────────────────────────────────────────────────────────
  // STEP 3: Process calendar blocks
  // ─────────────────────────────────────────────────────────────
  const calendarBlocks: Array<{ startsAt: Date; endsAt: Date; source: string; title: string | null }> = [];
  ...
  // ─────────────────────────────────────────────────────────────
  // STEP 4: Load or use shadow inventory
  // ─────────────────────────────────────────────────────────────
  let inventorySnapshot: InventorySnapshot[];
  let originalInventory: InventorySnapshot[];

  if (shadowInventory && shadowInventory.length > 0) {
    // Use shadow inventory for multi-day planning
    inventorySnapshot = shadowInventory
      .filter((item) => {
        if (!item.expirationDate) return true;
        const urgency = computeUrgency(item.expirationDate, todayLocal);
        return urgency >= 0;
      ...
      .map((item) => ({
        ...
        quantityConfidence: 'estimate' as const,
        unit: item.unit,
        expirationDate: item.expirationDate,
        createdAt: new Date(),
      }));
    ...
    inventorySnapshot = inventoryRaw
      .filter((item) => {
        if (!item.expirationDate) return true;
        const urgency = computeUrgency(item.expirationDate, todayLocal);
        return urgency >= 0;
  ...
  originalInventory = inventorySnapshot.map((i) => ({ ...i }));

  // Build inventory snapshot trace
  const inventorySnapshotTrace: InventorySnapshotTraceV05[] = inventorySnapshot.map((item) => {
    const expiresInDays = item.expirationDate
      ? differenceInDays(item.expirationDate, todayLocal)
      : null;
  ...
  for (const recipe of recipes) {
    ...
    if (excludeRecipeSlugs.includes(recipe.slug)) {
      candidates.push({
        recipeId: recipe.id,
        recipeSlug: recipe.slug,
        recipeName: recipe.name,
        totalTimeMinutes: totalTime,
        eligible: false,
        ineligibilityReason: 'Excluded (already planned in horizon or user-excluded)',
        missingRequired: [],
        usagePlan: [],
        scores: { wasteScore: 0, spendPenalty: 0, timePenalty: 0, final: -Infinity },
      });
      ...
      rejectedRecipesTrace.push({
        recipe: recipe.slug,
        eligible: false,
        reason: 'Excluded (already planned in horizon or user-excluded)',
      });
      continue;
    }
    ...
    // Check intent override hard constraints (must_exclude)
    if (intentOverride) {
      const ingredientNames = recipe.ingredients.map((i) => i.canonicalName);
      const recipeTags = Array.isArray(recipe.tags) ? recipe.tags : [];
      const intentCheck = checkRecipeMatchesIntent(
        { slug: recipe.slug, tags: recipeTags as string[], ingredientNames },
        intentOverride
      );

      if (intentCheck.hardFail) {
        candidates.push({
          recipeId: recipe.id,
          recipeSlug: recipe.slug,
          recipeName: recipe.name,
          totalTimeMinutes: totalTime,
          eligible: false,
          ineligibilityReason: 'Contains excluded ingredient (must_exclude)',
          missingRequired: [],
          usagePlan: [],
          scores: { wasteScore: 0, spendPenalty: 0, timePenalty: 0, final: -Infinity },
        });
        ...
        rejectedRecipesTrace.push({
          recipe: recipe.slug,
          eligible: false,
          reason: 'Contains excluded ingredient (must_exclude)',
        });
        continue;
      }
    }
    ...
    // ─────────────────────────────────────────────────────────────
    // v0.6: Apply variety penalty
    // ─────────────────────────────────────────────────────────────
    let varietyPenalty = 0;
    if (varietyProfile) {
      const ingredientNames = usagePlan.map((u) => u.canonicalName);
      const recipeTags = Array.isArray(recipe.tags) ? recipe.tags : [];
      const planningDate = todayLocal.toISOString().split('T')[0];
      ...
      varietyPenalty = varietyResult.totalPenalty;
    }

    // ─────────────────────────────────────────────────────────────
    // v0.6: Apply intent boost/penalty
    // ─────────────────────────────────────────────────────────────
    let intentBoost = 0;
    if (intentOverride) {
      const ingredientNames = recipe.ingredients.map((i) => i.canonicalName);
      ...
      intentBoost = intentCheck.boost;
    }

    // Final score with v0.6 adjustments
    const finalScore = wasteScore - spendPenalty - timePenalty - varietyPenalty + intentBoost;
  ...
  if (eligibleCandidates.length === 0) {
    ...
    for (const rejected of rejectedRecipesTrace) {
      ...
      if (rejected.reason.includes('Insufficient time')) {
        ...
        category = 'equipment';
      } else if (rejected.reason.includes('Excluded')) {
        category = 'excluded';
      } else if (rejected.reason.includes('must_exclude')) {
        category = 'intent_excluded';
      }
      rejectionCounts.set(category, (rejectionCounts.get(category) || 0) + 1);
    }
  ...
  eligibleCandidates.sort((a, b) => {
    ...
    if (b.scores.wasteScore !== a.scores.wasteScore) return b.scores.wasteScore - a.scores.wasteScore;
